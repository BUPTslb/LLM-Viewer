...
def get_num_key_value_heads(model_params):
    # Qwen models might use num_attention_heads for both query and key/value heads if not GQA/MQA
    # or they might have a specific attribute like num_key_value_heads
    # For now, assume it's the same as num_attention_heads if not explicitly defined
    return getattr(model_params, "num_key_value_heads", getattr(model_params, "num_attention_heads"))

def get_norm_layers(model_params):
    # Common norm layers in transformer blocks
...
def get_linear_layers(model_params, tp_size: int):
    ...
    # These are common linear layer names in transformer models.
    # Qwen might have slightly different names, but these are a good starting point.
    return {
        ...
        "o_proj":[hidden_size // tp_size, hidden_size], # Qwen often uses o_proj instead of out_proj
...
transformer_layer_graph={
    ...
    "o_proj":["sv_matmul"], # Changed from out_proj to o_proj
    "attn_add":["input","o_proj"],
    "mlp_norm":["attn_add"],
    "gate_proj":["mlp_norm"],
    "up_proj":["mlp_norm"],
...
flashattention_transformer_layer_graph={
    ...
    "o_proj":["fused_attention"], # Changed from out_proj to o_proj
    "attn_add":["input","o_proj"],
    "mlp_norm":["attn_add"],
    "gate_proj":["mlp_norm"],
    "up_proj":["mlp_norm"],
